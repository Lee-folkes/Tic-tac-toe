...
function setTargetIndex() {
  ...
  if (tileArray.indexOf("tile5") !== -1) {
    targetIndex = tileArray.indexOf("tile5");
    console.log("center chosen");
  }
  //prefer corners every turn
  else if (tileArray.indexOf(cornerToTry.classList[0]) !== -1) {
    targetIndex = tileArray.indexOf(cornerToTry.classList[0]);
    console.log("1: ", cornerToTry.classList[0]);
  } else if (tileArray.indexOf(cornerToTry.classList[0]) !== -1) {
    targetIndex = tileArray.indexOf(cornerToTry.classList[0]);
    console.log("2: ", cornerToTry.classList[0]);
  } else if (tileArray.indexOf(cornerToTry.classList[0]) !== -1) {
    targetIndex = tileArray.indexOf(cornerToTry.classList[0]);
    console.log("3: ", cornerToTry.classList[0]);
  } else if (tileArray.indexOf(cornerToTry.classList[0]) !== -1) {
    targetIndex = tileArray.indexOf(cornerToTry.classList[0]);
    console.log("4: ", cornerToTry.classList[0]);
  } else if (tileArray.indexOf(cornerToTry.classList[0]) !== -1) {
    targetIndex = tileArray.indexOf(cornerToTry.classList[0]);
    console.log("5: ", cornerToTry.classList[0]);
  }
  //if no corners then choose random free tile
  else {
    targetIndex = getRandomInt(0, tileArray.length);
  }
}
...
mainGrid.addEventListener("click", (e) => {
  ...
  if (playerOne.isCpu === false && playerTwo.isCpu === false) {
    if (
      e.target.classList.contains("hover-image") &&
